...
void aplicarXOR(unsigned char* pixelData, int width, int height, int incremento);
int main()
{
...
void aplicarXOR(unsigned char* pixelData, int width, int height, int incremento) {
    int total = width * height * 3; // 3 componentes de color por píxel (RGB)
    for (int i = 0; i < total; i++) {
        // Aplicar XOR entre el valor del píxel y el incremento
        pixelData[i] = pixelData[i] ^ incremento;
    }
}
